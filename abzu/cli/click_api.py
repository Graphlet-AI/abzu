    ...
    def get_company_facts(
        ...
        try:
            ...
            if error_response is not None:
                logger.error(f"Response: {error_response.text}")
            raise
...
def process_batch_companies(
    ...
    def process_company(company: Dict[str, Optional[str]]) -> Dict[str, Any]:
...
# Ignore the complexity issue for this function as it handles multiple scenarios
def facts(  # noqa: C901
...
def financialdatasets_facts_main(
    ticker: Optional[str] = None,
    cik: Optional[str] = None,
    input_file: Optional[str] = None,
    api_key: Optional[str] = None,
    ...
    Returns:
        0 on success, 1 on failure
    """
    # Call the main Click command function with the provided arguments
    result: int = facts(ticker, cik, input_file, api_key, pretty, output_file)
    return result
