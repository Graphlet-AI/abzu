"""Clean processed RSS files by backing them up and removing them."""
...
def clean():
    ...
    if not processed_dir.exists():
        click.echo(f"Processed directory {processed_dir} does not exist.")
        return

    # Find all files in processed directory (excluding hidden files)
    processed_files = [
        f for f in processed_dir.iterdir() if f.is_file() and not f.name.startswith(".")
    ]
    ...
    backup_dir = Path("/tmp/articles") / f"abzu_rss_backup_{timestamp}"
