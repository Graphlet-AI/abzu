...
class DataCenterPlaywrightCrawler:
    ...
    def __init__(self, output_path: str, max_pages: int = 300):
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    logger.error(f"Failed to access page at all: {url}")
                    return None
            ...
            try:
                ...
                logger.warning(f"Could not check page title for {url}")
    ...
    async def crawl(self) -> int:
        ...
        async with async_playwright() as p:
            ...
            article_page.on("console", lambda msg: logger.debug(f"Article page console: {msg.text}"))
                    ...
                    try:
                        ...
                        for attempt in range(3):
                            try:
                                ...
                                if attempt == 0:
                                    await list_page.goto(archive_url, wait_until="domcontentloaded", timeout=45000)
                                elif attempt == 1:
                                    await list_page.goto(archive_url, wait_until="networkidle", timeout=60000)
                                else:
                                ...
                                if title and "403" not in title.lower() and "error" not in title.lower():
                                    navigation_success = True
                                    logger.info(f"Successfully navigated to page {page_num} on attempt {attempt + 1}")
                                    break
                                else:
                                    logger.warning(f"Got blocked/error page on attempt {attempt + 1}: {title}")
                                    
                            except Exception as nav_error:
                                logger.warning(f"Navigation attempt {attempt + 1} failed for page {page_num}: {nav_error}")
                                if attempt < 2:
            ...
            with tqdm(total=len(all_article_urls), desc="Fetching articles", unit="article") as pbar:
...
def crawl_datacenter_playwright(
...
if __name__ == "__main__":
    # Test the crawler
    articles_count = crawl_datacenter_playwright(pages=2)
